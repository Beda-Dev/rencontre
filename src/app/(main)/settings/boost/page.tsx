    ...
    <div className="mx-auto w-full max-w-lg">
      ...
      <div className="grid grid-cols-2 gap-3 px-4 py-4">
        <div className="rounded-lg border border-white/10 bg-white/5 p-3 text-center">
          <p className="text-2xl font-semibold text-blue-400">{taps?.sent ?? 0}</p>
          <p className="text-xs text-white/50">Taps envoyés</p>
        </div>
        <div className="rounded-lg border border-white/10 bg-white/5 p-3 text-center">
          <p className="text-2xl font-semibold text-blue-400">{taps?.received ?? 0}</p>
          <p className="text-xs text-white/50">Taps reçus</p>
        </div>
      </div>

      {boost?.active && (
        <div className="mx-4 mb-4 rounded-lg border border-blue-400/30 bg-blue-400/10 p-3 text-sm text-blue-300">
      ...
      <div className="space-y-2 px-4 pb-8">
        {BOOST_OPTIONS.map((opt) => (
          <button
            ...
            className="flex w-full items-center justify-between gap-3 rounded-lg border border-white/10 bg-white/5 p-3 text-left hover:border-blue-400/40 disabled:opacity-50"
          >
            <span className="min-w-0 flex-1">
              <span className="block text-sm font-medium">{opt.label}</span>
              <span className="block text-xs text-white/50">{opt.desc}</span>
            </span>
            <span className="shrink-0 rounded-full bg-blue-400 px-3 py-1 text-xs font-medium text-black">
              Activer
            </span>
          </button>
        ))}
